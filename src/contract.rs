...
use crate::msg::{AllowanceAndTotalSupplyResponse, ExecuteMsg, InstantiateMsg, QueryMsg};
use crate::state::{MinterData, TokenInfo, ALLOWANCES, BALANCES, LOGO, MARKETING_INFO, TOKEN_INFO};

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:fx-lp";
...
pub fn update_minter(
    ...
    let origin_minter = config.mint.clone().unwrap().minter;
    if sender != origin_minter {
        return Err(ContractError::Std(StdError::generic_err("Only Minter")));
    };
    let minter = match new_minter {
        Some(minter) => deps.api.addr_validate(&minter)?,
        None => return Err(ContractError::Std(StdError::generic_err("Invalid addr"))),
    };

    config.mint = match config.mint.take() {
        Some(mut minter_data) => {
            minter_data.minter = minter;
            Some(minter_data)
        }
        None => Some(MinterData { minter, cap: None }),
    };
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::AllowanceAndTotalSuuply { owner, spender } => {
            to_binary(&query_allowance_and_total_supply(deps, owner, spender)?)
        }
...
pub fn query_allowance_and_total_supply(
    deps: Deps,
    owner: String,
    spender: String,
) -> StdResult<AllowanceAndTotalSupplyResponse> {
    ...
    let allowance = ALLOWANCES
        ...
        .unwrap_or_default();
    let info = TOKEN_INFO.load(deps.storage)?;
    let response = AllowanceAndTotalSupplyResponse {
